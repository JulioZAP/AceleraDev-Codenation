...
# Começa aqui
def main():
    st.image('logoec.jpeg', width= 600)
    st.title('AceleraDev - Semana 3')
    st.subheader('Semana 3 - Análise de dados exploratória')
    st.image('https://media.giphy.com/media/lkdIhnHHnFma6xvICt/giphy.gif', width=200)
    file = st.file_uploader('Faço o upload da base de dados desafio1 (.csv)', type='csv')
    if file is not None:
        df = pd.read_csv(file)
        st.subheader('Vamos começar vizualizando a base de dados selecionada.')
        numeros = st.slider('Note que limitamos o tamanho da visualização para obtermos maior performance na execução do app', min_value=5, max_value=30)
        ...
        st.image('https://media.giphy.com/media/l41YvpiA9uMWw5AMU/giphy.gif', width=200)
